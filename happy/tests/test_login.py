...
class TestFormLoginMiddleware(unittest.TestCase):
    PasswordBroker = DummyPasswordBroker
    PrincipalsBroker = DummyPrincipalsBroker
    CredentialBroker = DummyCredentialBroker
    ...
    def test_login_bad_password(self):
        ...
        request = Request.blank(
            '/login',
            POST=dict(login='chris@example.com', password='adfjdfkjk')
        )
        response = fut(request)
        self.assertEqual(response.status_int, 200)
        self.failUnless('Bad login' in response.body, response.body)
...
class TestHtpasswdAuthenticator(unittest.TestCase):
    def setUp(self):
        import os
        import tempfile
        fd, self.htpasswd_file = tempfile.mkstemp('.happy.test')
        os.close(fd)
...
class TestFlatFilePrincipalsBrokerTests(unittest.TestCase):
    def setUp(self):
        import os
        import tempfile
        self.fname = tempfile.mktemp('.happy.test')
...
class TestRefImplIntegration(TestFormLoginMiddleware):
    def setUp(self):
        import os
        import tempfile
        tmpdir = tempfile.mkdtemp('_happy_test')
        self.principals_db = os.path.join(tmpdir, 'principals')

        with open(self.principals_db, 'w') as f:
            print >>f, "# temp file for testing"
            print >>f, ""
        self._add_user_principals('chris@example.com', [
            'user-1234', 'group.Administrators'
            ]
        )

        self.htpasswd_file = os.path.join(tmpdir, 'htpasswd')
        with open(self.htpasswd_file, 'w') as f:
            print >>f, "# temp file for testing"
        self._add_user_password('chris@example.com', '12345678')

    def _add_user_principals(self, login, principals):
        import crypt
        principals = ', '.join(principals).encode('utf-8')
        with open(self.principals_db, 'a') as f:
            print >>f, '%s: %s' % (login, principals)
    ...
    def PasswordBroker(self):
        from happy.login import HtpasswdBroker
        return HtpasswdBroker(self.htpasswd_file)

    def PrincipalsBroker(self):
        from happy.login import FlatFilePrincipalsBroker
        return FlatFilePrincipalsBroker(self.principals_db)

    def CredentialBroker(self):
        from happy.login import RandomUUIDCredentialBroker
        return RandomUUIDCredentialBroker()
